...
function travelDay(mode='normal'){
  ...
  let calc=travelEstimate(mode),result=advanceRoute(calc.distance,mode,false);if(mode==='hurry')state.health=Math.max(0,(state.health??0)-1);
  t=ensureTravelState();let text=travelModeLabel(mode)+'：'+result.moved.toFixed(1)+'km進んだ。';if(result.event)text+=' '+result.event;if(result.arrived)text+=' '+state.location+'に到着した。';else t.phase='dusk';t.lastMode=mode;t.lastDayDistance=calc.distance;t.lastSummary=text;markDirty();render()
}
function campNight(){
  let t=ensureTravelState();if(!t||t.phase!=='dusk')return;
  state.health=Math.min(state.maxHealth??10,(state.health??0)+2);state.day=(state.day??1)+1;t.phase='ready';t.weather=null;t.weatherDay=null;t.dayVariance=null;t.lastSummary='野営して夜を越した。体力 +2。';markDirty();render()
}
function nightTravel(){
  ...
  let base=t.lastDayDistance||travelEstimate(t.lastMode||'normal')?.distance||12,extra=Math.max(3,Math.round(base*.28*10)/10),result=advanceRoute(extra,t.lastMode||'normal',true);state.health=Math.max(0,(state.health??0)-2);state.day=(state.day??1)+1;
  t=ensureTravelState();if(t.phase!=='settlement')t.phase='ready';t.weather=null;t.weatherDay=null;t.dayVariance=null;t.lastSummary='夜行してさらに '+result.moved.toFixed(1)+'km進んだ。体力 -2。'+(result.event?' '+result.event:'')+(result.arrived?' '+state.location+'に到着した。':'');markDirty();render()
}
function settlementNextDay(){let t=ensureTravelState();if(!t||!t.currentNodeId)return;state.health=Math.min(state.maxHealth??10,(state.health??0)+3);state.day=(state.day??1)+1;t.phase='ready';t.weather=null;t.weatherDay=null;t.dayVariance=null;t.lastSummary=state.location+'で一晩過ごした。体力 +3。';markDirty();render()}

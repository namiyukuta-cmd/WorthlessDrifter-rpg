...
function avatarImageUrl(src){if(!src)return null;let join=src.includes('?')?'&':'?';return src+join+'v='+GAME_ASSET_VERSION}
function loadAvatarImage(src){return new Promise((resolve,reject)=>{let img=new Image();img.decoding='async';img.onload=()=>resolve(img);img.onerror=reject;img.src=avatarImageUrl(src)})}
async function renderAvatarCanvas(){
  let canvas=$('avatarCanvas');if(!canvas)return;let seq=++avatarRenderSeq,ctx=canvas.getContext('2d');
  ctx.clearRect(0,0,canvas.width,canvas.height);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
  let hair=avatarChoice('hair'),sources=[hair?.back||null,AVATAR_DATA.base?.image||null,avatarChoice('clothes')?.image||null,avatarChoice('eyes')?.image||null,hair?.front||null].filter(Boolean);
  for(let src of sources){try{let img=await loadAvatarImage(src);if(seq!==avatarRenderSeq)return;ctx.drawImage(img,0,0,256,256)}catch(e){}}
}

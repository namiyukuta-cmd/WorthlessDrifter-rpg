...
function buildNewGameState(data){
  ...
  if(world){
    ...
    next.travel={
      worldId:world.id,
      currentNodeId:world.startNode,
      routeId:null,
      routeFrom:null,
      routeTo:null,
      routeProgressKm:0,
      knownNodes:[...(world.startingKnownNodes||[world.startNode])],
      knownRoutes:[...(world.startingKnownRoutes||[])],
      permits:[],
      phase:'ready',
      weather:null,
      weatherDay:null,
      lastSummary:'旅立ちの朝。まず行き先を決めよう。'
    };
  ...
  if(!def||def.id==='none'){
    ...
    next.companion={type:def.id,label:def.label,icon:def.icon,name:(data.companionName||'').trim(),health:def.health,maxHealth:def.health,dailyFood:def.dailyFood}
  }
  ...
  for(let it of next.items){
    if(it.parent?.startsWith('container:')){
      let parentId=it.parent.slice(10);
      if(!next.items.some(x=>x.id===parentId)){it.parent='loose';delete it.x;delete it.y}
    }
  }
  return next
}
